...
class DependencyManager {
	...
	private static function set_urls() {
		...
		self::$addon_title      = '<strong>KB Display As Blog Post Addon</strong>';
	}
	...
	private static function set_dependency_constants() {
		define( 'KBDABP_MIN_BKBM_VERSION', '1.5.7' );
		define( 'KBDABP_MIN_PHP_VERSION', '7.0' );
	}
	...
	public static function check_minimum_version_requirement_status() {
		...
		return ( version_compare( BKBM_CURRENT_PLUGIN_VERSION, KBDABP_MIN_BKBM_VERSION, '>=' ) );
	}
	...
	public static function notice_min_version_main_plugin() {
		...
		$message = sprintf(
            ...
            esc_html__( 'The %2$s requires %1$s %4$s or higher. You are using %3$s', 'bkb-kbdabp' ),
            self::$bkbm_url,
            self::$addon_title,
            BKBM_CURRENT_PLUGIN_VERSION,
            KBDABP_MIN_BKBM_VERSION
        );
	...
	public static function notice_missing_main_plugin() {
		...
		$message = sprintf(
            ...
            esc_html__( 'Please install and activate the %1$s plugin to use %2$s.', 'bkb-kbdabp' ),
            self::$bkbm_url,
            self::$addon_title
		);
	...
	public static function notice_missing_purchase_verification() {
		...
		$message = sprintf(
            ...
            esc_html__( 'Please Activate the %1$s to use the %2$s.', 'bkb-kbdabp' ),
