...
add_action("wp_ajax_bkbm_dabp_installation_counter", "bkbmDabpAddInstallationData");
add_action("wp_ajax_nopriv_bkbm_dabp_installation_counter", "bkbmDabpAddInstallationData");

function bkbmDabpApiUrl()
{
...
function bkbmDabpAddInstallationData()
{
  // bkbm_dabp_installation
  $apiURL = bkbmDabpApiUrl();
  $site_url = get_site_url();
  $product_id = BKBDABP_ADDON_CC_ID; // change the id
  $ip = $_SERVER['REMOTE_ADDR'];
  $ver = BKBDABP_ADDON_CURRENT_VERSION;
  ...
  if (is_array($output) && !is_wp_error($output) && wp_remote_retrieve_response_code($output) === 200) {
    ...
    if (isset($output_decode['status']) && $output_decode['status'] != 0) {
      ...
      update_option(BKBDABP_ADDON_INSTALLATION_TAG, '1'); // change the tag
