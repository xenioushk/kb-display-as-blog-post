...
namespace KBDABP\Callbacks\AdminAjaxHandlers;

use KBDABP\Traits\PluginInstallationTraits;
	...
	public function save() {
		...
		if ( is_array( $output ) && ! is_wp_error( $output ) && wp_remote_retrieve_response_code( $output ) === 200 ) {
			...
			if ( isset( $output_decode['status'] ) && $output_decode['status'] != 0 ) {
				...
				update_option( KBDABP_PRODUCT_INSTALLATION_TAG, 1 ); // change the tag
